...
class HapticForceController:
    ...
    def __init__(self, delay_time=0.1):
    ...
    def forcevector_conversion(joint_position_robot, robot_force):
        # Force vector conversion with gravity and correction factor
    ...
    def robot_force_callback(self, msg: WrenchStamped):
        # Buffer data
        self.force_window.append([msg.wrench.force.x, msg.wrench.force.y, msg.wrench.force.z])
        self.robot_force = np.mean(self.force_window, axis=0)
        self.delay_buffer.append(self.forcevector_conversion(self.joint_position_robot, self.robot_force))
        

    def haptic_force_callback(self, event):
        if len(self.delay_buffer) > 0:
            delayed_force = self.delay_buffer.popleft()  # Get delayed force
            rospy.loginfo("Publishing delayed force: {}".format(delayed_force))
            force_pub_msg = OmniFeedback()
            force_pub_msg.force.x, force_pub_msg.force.y, force_pub_msg.force.z = delayed_force
    ...
    def make_csv(self):
        with open('haptic_force_data.csv', 'w', newline='') as csvfile:
            fieldnames = ['Time', 'Robot Force X', 'Robot Force Y', 'Robot Force Z', 'Haptic Force X', 'Haptic Force Y', 'Haptic Force Z']
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            writer.writeheader()
            for t, rf, hf in zip(self.time_stamps, self.robot_force_plot, self.haptic_force_plot):
                writer.writerow({'Time': t, 'Robot Force X': rf[0], 'Robot Force Y': rf[1], 'Robot Force Z': rf[2],
                                'Haptic Force X': hf[0], 'Haptic Force Y': hf[1], 'Haptic Force Z': hf[2]})
