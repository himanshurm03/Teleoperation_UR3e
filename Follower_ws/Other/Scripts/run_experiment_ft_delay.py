...
class HapticForceController:
    ...
    def __init__(self):
        ...
        self.ft_time_slave_pub = rospy.Publisher('ft_time_slave', String, queue_size=1)
        ...
        rospy.Subscriber('/ft_time_master', String, self.ft_time_master_callback)
        ...
        self.time_stamps_exp = None
        self.force_exp_exp = None

        self.file_path_recieve = '/home/user/experiment_data/standard2_v2/force_data_for_exp_interpolated.csv'
        self.file_path_store = '/home/user/experiment_data/22_sept_2024/RoboLab_hussernet_circle_v1_k8.5/force_data.csv'

        # For delay
        self.first_string = '0'
        self.round_way = 0.0
        self.t_init = 0.0
        self.teleop_file_ur3e = "Teleoperation file"
        self.js_start = True

    def ft_time_master_callback(self, msg5):
        time_data = msg5.data
        separated_string = time_data.split(" ")
        self.first_string = separated_string[0]
    ...
    def robot_force_callback(self, msg: WrenchStamped):
        ...
        if self.start_time_exp is None:
            self.robot_callback_dataset()
            self.start_time_exp = current_time
        current_time = current_time - self.start_time_exp

        time_stamps = self.time_stamps_exp
        poses = self.force_exp

        if self.time_stamps_exp is None or self.force_exp is None:
            rospy.logerr("Data has not been loaded properly.")
            return

        if current_time <= time_stamps[-1]:
            idx = np.searchsorted(time_stamps, current_time, side='right') - 1
            if idx < 0:
                idx = 0 
            self.robot_force = np.array(poses[idx])
        else:
            return

        return self.robot_force

    def robot_callback_dataset(self):
        time_stamps = []
        force = []
        with open(self.file_path_recieve, 'r') as csvfile:
            csvreader = csv.reader(csvfile)
            next(csvreader)  # Skip header
            for row in csvreader:
                time_stamps.append(float(row[0]))
                force_values = list(map(float, row[1:]))  # Convert the force values to floats
                force.append(force_values)  # Append the converted values to the list

        self.time_stamps_exp = np.array(time_stamps)
        self.force_exp = np.array(force)
    ...
    def haptic_force_callback(self, event):
        ...
        t_now_1 += " " + self.first_string
        self.ft_time_slave_pub.publish(t_now_1) 
    ...
    def make_csv(self):
        with open(self.file_path_store, 'w', newline='') as csvfile:
