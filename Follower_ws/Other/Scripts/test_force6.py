...
class HapticForceController:
    ...
    def __init__(self):
        ...
        self.force_sensor_receive_time = None
        self.force_processing_log = []  # (sensor_time, send_time, diff)
    ...
    def robot_force_callback(self, msg: WrenchStamped):
        self.force_sensor_receive_time = rospy.get_time()
    ...
    def haptic_force_callback(self, event):
        ...
        # Logging
        generation_time = rospy.get_time()
        force_timestamp = generation_time
        self.internal_force_timestamp_pub.publish(Float64(force_timestamp))
        self.sent_force_times[force_timestamp] = generation_time
        send_time = rospy.get_time()

        if self.force_sensor_receive_time is not None:
            diff = send_time - self.force_sensor_receive_time
            self.force_processing_log.append((
                self.force_sensor_receive_time,
                send_time,
                diff
            ))

        self.force_gen_send_log.append((
            generation_time,
            send_time,
            send_time - generation_time
        ))
    ...
    def make_csv_force_processing(self):
        with open('/home/user/Desktop/delay/force_computation.csv', 'w', newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow([
                'Sensor Receive Time (s)',
                'Sent to Master Time (s)',
                'Processing Delay (s)'
            ])
            for recv, sent, diff in self.force_processing_log:
                writer.writerow([
                    f"{recv:.9f}",
                    f"{sent:.9f}",
                    f"{diff:.9f}"
    ...
    def shutdown_hook(self):
        ...
        self.make_csv_force_processing()
...
#         self.start_time = time.time() 
#         self.robot_force = np.zeros(3)
#         self.robot_force_raw = np.zeros(3)  # Store raw force separately
#         self.robot_force_initial = np.zeros(3)
#         self.joint_position_robot = np.zeros(6)
#         self.haptic_force = np.zeros(3)
#         self.haptic_force_prev = np.zeros(3)
...
#         # CRITICAL: Dual-stage filtering for viscoelastic materials
#         # Stage 1: Fast moving average for noise reduction (minimal lag)
#         self.fast_ma_window_size = 5  # 10ms at 500Hz - very responsive
#         self.fast_ma_window = deque(maxlen=self.fast_ma_window_size)
        
#         # Stage 2: Adaptive low-pass filter for stability
#         self.use_adaptive_filter = True
#         self.alpha_base = 0.3  # Base smoothing factor
#         self.alpha = self.alpha_base  # Will be adapted based on force rate of change
        
#         # Passivity-based damping to prevent energy injection
#         self.passivity_damping_enabled = True
#         self.damping_gain = 0.15  # Critical damping coefficient
#         self.energy_tank = 0.0  # Energy observer for passivity
#         self.max_energy_tank = 5.0  # Maximum stored energy
        
#         # Force rate limiter to prevent sudden jumps (critical for stability)
#         self.force_rate_limit_enabled = True
#         self.max_force_rate = 15.0  # N/s maximum force change rate
#         self.dt = 1.0 / 500.0  # 500Hz sample time
        
#         # Viscoelastic material detection and adaptation
#         self.material_stiffness_estimate = 1.0  # Estimated environment stiffness
#         self.stiffness_history = deque(maxlen=20)
#         self.is_soft_contact = False
        
#         # Deadband and saturation
#         self.force_deadband = 0.08  # Lower threshold for better sensitivity
#         self.force_saturation = 8.0  # Maximum force to prevent instability

#         # Initialization flag
#         self.initialized = False
#         self.init_sample_count = 0
#         self.init_samples_needed = 100  # More samples for better initialization
...
#         self.robot_force_plot = []
#         self.haptic_force_plot = []
#         self.alpha_log = []  # Log adaptive filter coefficient
#         self.damping_log = []  # Log damping forces

#         # Weber sampling - FIXED implementation with hysteresis
#         self.weber_enabled = True
#         self.weber_delta = 0.06  # Slightly reduced for better responsiveness
#         self.weber_hysteresis = 0.02  # Hysteresis to prevent chattering
#         self.prev_weber_force_raw = None
#         self.last_sent_force = None
        
#         # Performance monitoring
#         self.force_change_rate = 0.0
#         self.instability_counter = 0
        
#         rospy.loginfo("="*70)
#         rospy.loginfo("ADVANCED HAPTIC FORCE CONTROLLER FOR VISCOELASTIC MATERIALS")
#         rospy.loginfo("="*70)
#         rospy.loginfo("Dual-stage filtering: Fast MA (5 samples) + Adaptive LPF")
#         rospy.loginfo("Passivity-based damping: ENABLED")
#         rospy.loginfo("Force rate limiting: %.1f N/s", self.max_force_rate)
#         rospy.loginfo("Weber sampling with hysteresis: %.3f / %.3f", self.weber_delta, self.weber_hysteresis)
#         rospy.loginfo("Deadband: %.2f N | Saturation: %.1f N", self.force_deadband, self.force_saturation)
#         rospy.loginfo("="*70)

#     def master_echo_callback(self, msg: MasterToSlaveData):
#         """Receives data FROM master to calculate RTD"""
#         force_timestamp_echo = msg.force_timestamp
...
#             rospy.loginfo_throttle(2.0, f"Force RTD: {rtd*1000:.2f}ms | Alpha: {self.alpha:.3f} | Soft: {self.is_soft_contact}")
            
#             del self.sent_force_times[force_timestamp_echo]

#     def estimate_material_stiffness(self, force_magnitude, force_rate):
#         """
#         Estimate material stiffness from force dynamics
#         Soft viscoelastic materials: low stiffness + high damping
#         """
#         if force_magnitude < self.force_deadband:
#             return
        
#         # Simple stiffness indicator based on force rate
#         # Soft materials: gradual force buildup (low rate)
#         # Stiff materials: rapid force buildup (high rate)
#         if abs(force_rate) > 1e-3:
#             stiffness_indicator = force_magnitude / (abs(force_rate) + 1e-6)
#             self.stiffness_history.append(stiffness_indicator)
            
#             if len(self.stiffness_history) >= 10:
#                 avg_stiffness = np.mean(self.stiffness_history)
#                 # Soft contact threshold - tune based on your materials
#                 self.is_soft_contact = avg_stiffness < 3.0

#     def adaptive_low_pass_filter(self, current_force, prev_force):
#         """
#         Adaptive first-order low-pass filter
#         Alpha adapts based on force change rate
#         High change rate -> lower alpha (more smoothing for stability)
#         Low change rate -> higher alpha (more responsiveness)
#         """
#         if not self.use_adaptive_filter:
#             return current_force
        
#         # Calculate force change rate
#         force_change = np.linalg.norm(current_force - prev_force)
#         self.force_change_rate = force_change / self.dt
        
#         # Adaptive alpha based on change rate and material detection
#         if self.is_soft_contact:
#             # For soft materials: more aggressive smoothing
#             if self.force_change_rate > 10.0:
#                 self.alpha = 0.15  # Heavy smoothing
#             elif self.force_change_rate > 5.0:
#                 self.alpha = 0.25
#             else:
#                 self.alpha = 0.35
#         else:
#             # For stiff materials: less smoothing
#             if self.force_change_rate > 20.0:
#                 self.alpha = 0.25
#             elif self.force_change_rate > 10.0:
#                 self.alpha = 0.40
#             else:
#                 self.alpha = 0.50
        
#         # Apply low-pass filter: y[n] = alpha * x[n] + (1-alpha) * y[n-1]
#         filtered_force = self.alpha * current_force + (1.0 - self.alpha) * prev_force
        
#         return filtered_force

#     def apply_passivity_damping(self, current_force, prev_force):
#         """
#         Passivity-based damping to ensure system stability
#         Monitors energy flow and adds damping when needed
#         Prevents energy injection that causes oscillations
#         """
#         if not self.passivity_damping_enabled:
#             return current_force, 0.0
        
#         # Calculate force velocity (rate of change)
#         force_velocity = (current_force - prev_force) / self.dt
        
#         # Energy observation (simplified)
#         power_flow = np.dot(current_force, force_velocity)
#         self.energy_tank += power_flow * self.dt
        
#         # Limit energy tank
#         self.energy_tank = np.clip(self.energy_tank, -self.max_energy_tank, self.max_energy_tank)
        
#         # Apply damping when energy is being added to system
#         damping_force = np.zeros(3)
#         if self.energy_tank > 0.1 or power_flow > 0.5:
#             # Proportional damping based on velocity
#             damping_force = -self.damping_gain * force_velocity
            
#             # Extra damping for soft contact (more prone to oscillation)
#             if self.is_soft_contact:
#                 damping_force *= 1.5
        
#         damped_force = current_force + damping_force
        
#         return damped_force, np.linalg.norm(damping_force)

#     def apply_force_rate_limiter(self, desired_force, prev_force):
#         """
#         Limit the rate of force change to prevent sudden jumps
#         Critical for preventing instability with soft materials
#         """
#         if not self.force_rate_limit_enabled:
#             return desired_force
        
#         force_diff = desired_force - prev_force
#         force_diff_magnitude = np.linalg.norm(force_diff)
        
#         max_change = self.max_force_rate * self.dt
        
#         if force_diff_magnitude > max_change:
#             # Limit the change
#             limited_diff = force_diff * (max_change / force_diff_magnitude)
#             return prev_force + limited_diff
#         else:
#             return desired_force

#     def apply_weber_sampling_with_hysteresis(self, current_force):
#         """
#         Weber sampling with hysteresis to prevent chattering
#         Dual threshold: weber_delta to trigger change, hysteresis to confirm
#         """
#         if not self.weber_enabled:
...
#         # Initialize
#         if self.prev_weber_force_raw is None:
#             self.prev_weber_force_raw = current_force.copy()
#             self.last_sent_force = current_force.copy()
#             return current_force.copy(), True

#         # Always update raw reference
#         Fn = current_force
#         Fp_raw = self.prev_weber_force_raw
#         Fp_sent = self.last_sent_force
        
#         self.prev_weber_force_raw = Fn.copy()
        
#         # Calculate change relative to last SENT force
#         eps = 1e-6
#         ratio = np.linalg.norm(Fn - Fp_sent) / (np.linalg.norm(Fp_sent) + eps)
        
#         # Hysteresis logic
#         threshold = self.weber_delta
#         if ratio >= threshold + self.weber_hysteresis:
#             # Definitely send
#             self.last_sent_force = Fn.copy()
#             return Fn.copy(), True
#         elif ratio <= threshold - self.weber_hysteresis:
#             # Definitely don't send new value
#             return Fp_sent.copy(), False
#         else:
#             # In hysteresis band - keep previous decision
#             return Fp_sent.copy(), False
...
#         robot_force_base = self.forcevector_conversion(
#             self.joint_position_robot, 
#             self.robot_force_raw  # Use raw for comparison
#         )
#         self.robot_force_plot.append(robot_force_base)
#         self.haptic_force_plot.append(self.haptic_force.copy())
#         self.alpha_log.append(self.alpha)

#     @staticmethod
#     def forcevector_conversion(joint_position_robot, robot_force):
#         """Convert force from sensor frame to base frame with gravity compensation"""
#         q1, q2, q3, q4, q5, q6 = joint_position_robot
#         cos = np.cos
#         sin = np.sin
...
#         if magnitude_of_cross_product_vector > 1e-6:
#             unit_cross_product_base = cross_product / magnitude_of_cross_product_vector
#             unit_cross_product_sensor = R.T @ unit_cross_product_base
#             angle = np.arctan2(magnitude_of_cross_product_vector, dot_product)
#             correction_factor = (1.34+0.15)*sin(angle)*unit_cross_product_sensor
#         else:
#             correction_factor = np.zeros(3)
...
#     def robot_force_callback(self, msg: WrenchStamped):
#         """Receive force/torque data - Stage 1: Fast moving average"""
#         robot_force = np.array([msg.wrench.force.x, msg.wrench.force.y, msg.wrench.force.z])
#         self.fast_ma_window.append(robot_force)
        
#         # Fast moving average for noise reduction
#         if len(self.fast_ma_window) >= self.fast_ma_window_size:
#             self.robot_force_raw = np.mean(self.fast_ma_window, axis=0)
#         else:
#             self.robot_force_raw = robot_force
        
#         # Initialization
#         if not self.initialized:
#             self.init_sample_count += 1
#             if self.init_sample_count >= self.init_samples_needed:
#                 self.initialized = True
#                 self.haptic_force_prev = np.zeros(3)
#                 rospy.loginfo("Force controller initialized - ready for operation")

#     def haptic_force_callback(self, event):
#         """Main force control loop - runs at 500Hz with advanced filtering"""
#         if self.shutdown_flag or not self.initialized:
#             return

#         # Step 1: Convert to base frame
#         robot_force_base = self.forcevector_conversion(
#             self.joint_position_robot,
#             self.robot_force_raw
#         )
        
#         # Step 2: Adaptive low-pass filtering (Stage 2)
#         force_filtered = self.adaptive_low_pass_filter(
#             robot_force_base, 
#             self.haptic_force_prev
#         )
        
#         # Step 3: Material stiffness estimation
#         force_magnitude = np.linalg.norm(force_filtered)
#         self.estimate_material_stiffness(force_magnitude, self.force_change_rate)
        
#         # Step 4: Passivity-based damping
#         force_damped, damping_magnitude = self.apply_passivity_damping(
#             force_filtered,
#             self.haptic_force_prev
#         )
        
#         # Step 5: Force rate limiting
#         force_limited = self.apply_force_rate_limiter(
#             force_damped,
#             self.haptic_force_prev
#         )
        
#         # Step 6: Saturation
#         force_magnitude_limited = np.linalg.norm(force_limited)
#         if force_magnitude_limited > self.force_saturation:
#             force_limited = force_limited * (self.force_saturation / force_magnitude_limited)
        
#         # Step 7: Weber sampling with hysteresis
#         sampled_force, should_send = self.apply_weber_sampling_with_hysteresis(force_limited)
        
#         # Step 8: Publish force
#         force_msg = Vector3()
#         force_msg.x = sampled_force[0]
#         force_msg.y = sampled_force[1]
#         force_msg.z = sampled_force[2]
        
#         self.internal_force_pub.publish(force_msg)

#         # Step 9: Publish timestamp
#         force_timestamp = rospy.get_time()
#         self.internal_force_timestamp_pub.publish(Float64(force_timestamp))

#         # Step 10: Store for RTD
...
#         self.damping_log.append(damping_magnitude)

#         # Step 12: Update state
#         self.haptic_force = sampled_force.copy()
#         self.haptic_force_prev = force_limited.copy()  # Use limited for next iteration
#         self.robot_force = robot_force_base.copy()  # Store for plotting

#         # Step 13: Update plots
#         self.update_list()

#     def plot_data(self):
#         """Generate comprehensive force analysis plots"""
#         times = np.array(self.time_stamps)
#         robot_force_plot = np.array(self.robot_force_plot)
#         haptic_force_plot = np.array(self.haptic_force_plot)
#         alpha_log = np.array(self.alpha_log)

#         fig, axs = plt.subplots(4, 2, figsize=(16, 12))
        
#         # Force comparison plots
#         for i, axis_name in enumerate(['X', 'Y', 'Z']):
#             axs[i, 0].plot(times, robot_force_plot[:, i], label='Raw Robot Force', alpha=0.5, linewidth=1)
#             axs[i, 0].plot(times, haptic_force_plot[:, i], label='Filtered Haptic Force', linewidth=2)
#             axs[i, 0].set_title(f'Force in {axis_name}')
#             axs[i, 0].set_ylabel('Newtons')
#             axs[i, 0].set_xlabel('Seconds')
#             axs[i, 0].grid(True, alpha=0.3)
#             axs[i, 0].legend()

#             # Error plots
#             axs[i, 1].plot(times, robot_force_plot[:, i] - haptic_force_plot[:, i])
#             axs[i, 1].set_title(f'Filtering Effect in {axis_name}')
#             axs[i, 1].set_ylabel('Force Difference (N)')
#             axs[i, 1].set_xlabel('Seconds')
#             axs[i, 1].grid(True, alpha=0.3)
        
#         # Adaptive filter coefficient
#         axs[3, 0].plot(times, alpha_log, color='orange', linewidth=2)
#         axs[3, 0].set_title('Adaptive Filter Coefficient (Alpha)')
#         axs[3, 0].set_ylabel('Alpha')
#         axs[3, 0].set_xlabel('Seconds')
#         axs[3, 0].grid(True, alpha=0.3)
#         axs[3, 0].set_ylim([0, 1])
        
#         # Force magnitude comparison
#         robot_mag = np.linalg.norm(robot_force_plot, axis=1)
#         haptic_mag = np.linalg.norm(haptic_force_plot, axis=1)
#         axs[3, 1].plot(times, robot_mag, label='Raw', alpha=0.5)
#         axs[3, 1].plot(times, haptic_mag, label='Filtered', linewidth=2)
#         axs[3, 1].set_title('Force Magnitude')
#         axs[3, 1].set_ylabel('Newtons')
#         axs[3, 1].set_xlabel('Seconds')
#         axs[3, 1].grid(True, alpha=0.3)
#         axs[3, 1].legend()

#         plt.tight_layout()
#         plt.savefig('/home/user/Desktop/delay/force_analysis.png', dpi=150)
#         plt.show()
...
#             for i in range(len(self.time_stamps)):
#                 ts_sent = self.force_sent_timestamps[i] if i < len(self.force_sent_timestamps) else ''
#                 alpha_val = self.alpha_log[i] if i < len(self.alpha_log) else ''
#                 damp_val = self.damping_log[i] if i < len(self.damping_log) else ''
#                 writer.writerow([
#                     f"{ts_sent:.9f}" if ts_sent != '' else '',
#                     f"{self.time_stamps[i]:.9f}",
#                     f"{alpha_val:.4f}" if alpha_val != '' else '',
#                     self.robot_force_plot[i][0], self.robot_force_plot[i][1], self.robot_force_plot[i][2],
#                     self.haptic_force_plot[i][0], self.haptic_force_plot[i][1], self.haptic_force_plot[i][2],
#                     f"{damp_val:.4f}" if damp_val != '' else ''
#                 ])
#         rospy.loginfo("Force data saved to: /home/user/Desktop/delay/force.csv")

#     def make_csv_for_generation_and_send(self):
#         """Save force generation timing"""
#         with open('/home/user/Desktop/delay/force_gen_send.csv', 'w', newline='') as csvfile:
#             writer = csv.writer(csvfile)
#             writer.writerow(['Generation Time (s)', 'Sent Timestamp (ROS Time)', 'Difference (s)'])
#             for gen_time, sent_time, diff in self.force_gen_send_log:
#                 writer.writerow([f"{gen_time:.9f}", f"{sent_time:.9f}", f"{diff:.9f}"])
#         rospy.loginfo("Generation/send log saved")

#     def make_csv_for_force_rtd(self):
#         """Save force RTD with statistics"""
#         with open('/home/user/Desktop/delay/force_rtd.csv', 'w', newline='') as csvfile:
#             writer = csv.writer(csvfile)
#             writer.writerow(['Force Timestamp', 'Force RTD (s)'])
#             for timestamp, rtd in self.force_rtd_log:
...
#             rospy.loginfo("FORCE RTD STATISTICS:")
#             rospy.loginfo(f"  Average: {np.mean(rtds)*1000:.3f} ms")
#             rospy.loginfo(f"  Median:  {np.median(rtds)*1000:.3f} ms")
#             rospy.loginfo(f"  Min:     {np.min(rtds)*1000:.3f} ms")
#             rospy.loginfo(f"  Max:     {np.max(rtds)*1000:.3f} ms")
#             rospy.loginfo(f"  Std Dev: {np.std(rtds)*1000:.3f} ms")
#             rospy.loginfo(f"  Samples: {len(rtds)}")
#             rospy.loginfo("="*60)
        
#         rospy.loginfo("RTD log saved to: /home/user/Desktop/delay/force_rtd.csv")
...
#         zero_force_msg = Vector3()
#         zero_force_msg.x, zero_force_msg.y, zero_force_msg.z = (0, 0, 0)
#         rospy.loginfo("Sending zero force commands...")
#         for _ in range(15):
#             self.internal_force_pub.publish(zero_force_msg)
#             time.sleep(0.05)
...
#         rospy.loginfo("All logs saved successfully")
