...
class HapticForceController:
    ...
    def __init__(self):
        ...
        # Publishers
        self.force_controller_pub = rospy.Publisher('displacement_for_impedance', Float64MultiArray, queue_size=1)
    ...
    def forcevector_conversion(joint_position_robot, robot_force):
        ...
        F_offset = np.array([0, 0, -3.834+0.51])
    ...
    def control_law(self, robot_force):
        ...
        if force_z<-tau and self.controller_flag:
            self.displacement_z = (force_z+tau)/k
        # if abs(self.displacement_x) > 0.0002 or abs(self.displacement_y) > 0.0002 or abs(self.displacement_z) > 0.0002:
        #     force_pub_msg = Float64MultiArray()
        #     #rospy.loginfo(f"Displacement: x={self.displacement_x}, y={self.displacement_y}, z={self.displacement_z}")
        #     force_pub_msg.data = [self.displacement_x, self.displacement_y, self.displacement_z]
        #     self.force_controller_pub.publish(force_pub_msg)
        force_pub_msg = Float64MultiArray()
        #rospy.loginfo(f"Displacement: x={self.displacement_x}, y={self.displacement_y}, z={self.displacement_z}")
        force_pub_msg.data = [self.displacement_x, self.displacement_y, self.displacement_z]
        self.force_controller_pub.publish(force_pub_msg)
