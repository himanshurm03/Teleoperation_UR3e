...
def haptic_callback(msg: PoseStamped, context):
    ...
    haptic_stylus_position, h_offset_array, flag_pos_offset = context
...
def haptic_offset(msg, context):
    ...
    return haptic_current_angular_pose
...
def desired_traj(haptic_current_linear_pose, haptic_current_angular_pose, current_ee_pose):
    ...
    desired_ee_trajectory = end_effector_start + haptic_current_pose
...
def main():
    ...
    haptic_context = (haptic_stylus_position, h_offset_array, flag_pos_offset)

    joint_position_context = []

    th1_h_off, th2_h_off, th3_h_off, th4_h_off, th5_h_off, th6_h_off = [np.zeros(1) for _ in range(6)]
    flag_offset = [True]
    haptic_offset_context = (th1_h_off, th2_h_off, th3_h_off, th4_h_off, th5_h_off, th6_h_off, flag_offset)

    current_ee_pose = rospy.Subscriber('/tf', TFMessage, tf_callback, callback_args=tf_context)
    haptic_current_linear_pose = rospy.Subscriber('/phantom/phantom/pose', PoseStamped, haptic_callback, callback_args=haptic_context)
    joint_position_robot = rospy.Subscriber('/joint_states', JointState, joint_callback_robot, callback_args=joint_position_context)
    haptic_current_angular_pose = rospy.Subscriber('/joint_states', JointState, haptic_offset, callback_args=haptic_offset_context)
    ...
    velocity_pub_msg.data = rpy2joint_space_vel(0, 0, 0, current_ee_pose, desired_ee_trajectory, joint_position_robot) 
