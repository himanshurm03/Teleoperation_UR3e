#!/usr/bin/env python3
#THIS LOGS THE RAW FORCE BUT SEND ZERO HAPTIC FORCE
# import rospy
# import numpy as np
...
#     # Prepare and publish ZERO force
#         force_pub_msg = OmniFeedback()
...
#     # Store zero haptic force for logging/plotting
#         self.haptic_force = np.array([0.0, 0.0, 0.0])
#         self.update_list()
...
class HapticForceController:
    ...
    def forcevector_conversion(joint_position_robot, robot_force):
        ...
        # can be added 0.2 in the z component
        # F_offset = np.array([0, 0, -3.834+0.25+1.5])
        F_offset = np.array([0, 0, -3.834+0.51-0.4+0.02+0.04]) # USE THIS IF NEEDED

        # robot_force = robot_force - g_sensor - correction_factor
        robot_force = robot_force - g_sensor - F_offset - correction_factor
        return (np.dot(R, robot_force))
    ...
    def robot_force_callback(self, msg: WrenchStamped):
        ...
        self.force_window.append(robot_force)
        self.robot_force = np.mean(self.force_window, axis=0)
        #self.robot_force = np.array([msg.wrench.force.x, msg.wrench.force.y, msg.wrench.force.z])  #Same as Robot force
