#!/usr/bin/env python3
#Purpose:Constant force during interaction
...
class HapticForceController:
    ...
    def __init__(self):
        ...
        self.haptic_window_size = 10
        self.force_window = deque(maxlen=self.haptic_window_size)

        # NEW: Constant force parameters 
        self.contact_threshold = 1.5  
        self.constant_force_magnitude = 2.0  
        self.in_contact = False
        self.contact_direction = np.zeros(3)  
        ...
        rospy.loginfo("Constant force mode: 2N during contact")
        rospy.loginfo(f"Contact threshold: {self.contact_threshold}N")

    def master_echo_callback(self, msg: MasterToSlaveData):
        """Receives bundled data from master for RTD calculation"""
    ...
    def haptic_force_callback(self, event):
        ...
        # Get the actual robot force (in base frame)
        robot_force = self.forcevector_conversion(self.joint_position_robot, self.robot_force)
        force_magnitude = np.linalg.norm(robot_force)

        # ===== NEW: Constant force logic =====
        # Detect contact based on threshold
        if force_magnitude > self.contact_threshold:
            if not self.in_contact:
                # Just made contact - capture the direction
                self.in_contact = True
                self.contact_direction = robot_force / force_magnitude  # Normalize
                rospy.loginfo(f"Contact detected! Force: {force_magnitude:.2f}N")
            
            # Apply constant force in the contact direction
            output_force = self.contact_direction * self.constant_force_magnitude
            rospy.loginfo_throttle(0.5, f"Constant force active: 2.0N (actual: {force_magnitude:.2f}N)")
        
        else:
            # No contact - send zero force
            if self.in_contact:
                rospy.loginfo("Contact lost")
            self.in_contact = False
    ...
    def plot_data(self):
        ...
        axs[0, 0].plot(times, haptic_force_plot[:, 0], label='haptic force (constant 2N)')
        ...
        axs[1, 0].plot(times, haptic_force_plot[:, 1], label='haptic force (constant 2N)')
        ...
        axs[2, 0].plot(times, haptic_force_plot[:, 2], label='haptic force (constant 2N)')
...
#         """Callback to receive timestamps back from master and compute round-trip delay"""
...
#     # def haptic_force_callback(self, event):
#     #     """Instead of sending forces, just send timestamp to master"""
#     #     if self.shutdown_flag:
#     #         return
...
#     def haptic_force_callback(self, event=None):
#         if self.shutdown_flag:
#             return
...
#         """Save computed round-trip delays"""
#         filepath = "/home/user/Desktop/delay/forced.csv"
#         with open(filepath, 'w', newline='') as csvfile:
#             writer = csv.writer(csvfile)
#             writer.writerow(['Timestamp sent to master', 'Round-trip delay (s)'])
...
#             self.haptic_force_callback(None)  # manually call force publisher
#             rate.sleep()

#     def shutdown_hook(self):
#         self.shutdown_flag = True
#         rospy.loginfo("Shutting down force controller...")
