...
#8
#ABOVE CODE PERFECTLY FINE
#PURPOSE:TO LOG THE FORCE APPLIED TO THE HAPTIC DEVICE
...
#     def save_force_applied_to_csv(self, filepath="/home/autonomous-lab/Desktop/delay/force_applied.csv"):
#         try:
#             os.makedirs(os.path.dirname(filepath), exist_ok=True)
#             with open(filepath, 'w', newline='') as csvfile:
#                 writer = csv.writer(csvfile)
...
#                     'Force Z (N)'
#                 ])
#                 for t, fx, fy, fz in self.force_applied_log:
#                     writer.writerow([
#                         f"{t:.6f}",
#                         f"{fx:.6f}",
#                         f"{fy:.6f}",
#                         f"{fz:.6f}"
#                     ])
#             rospy.loginfo(f"Force applied log saved to: {filepath}")
#         except Exception as e:
#             rospy.logerr(f"Failed to write force applied log: {e}")
...
#         self.save_force_receive_to_csv()
#         self.save_force_timestamp_wait_to_csv()
#         self.save_force_applied_to_csv()
...
#10
#to resolve the issue of increment in the pose rtd for master at 1000Hz
...
#         # Track latest force data
#         self.latest_force_data = None

#         # Track force timestamp waiting time with dictionary
...
#         force_timestamp = msg.force_timestamp
#         if force_timestamp > 0:  # Valid timestamp
#             # Only record FIRST receive time for each unique force timestamp
#             if force_timestamp not in self.force_timestamp_first_receive:
#                 self.force_timestamp_first_receive[force_timestamp] = rospy.get_time()
#                 rospy.loginfo_throttle(2.0, f"NEW force_timestamp received: {force_timestamp:.6f}")
...
#         """
#         Callback for pose messages from haptic device.
#         ✅ FIXED: Now timestamps when MASTER SENDS, not haptic generation time
#         """
#         # Increment counter
#         self.sample_counter += 1
        
#         # Only process every DOWNSAMPLE_FACTOR sample
#         if self.sample_counter >= self.DOWNSAMPLE_FACTOR:
#             self.latest_pose_msg = msg
#             self.sample_counter = 0
...
#             # ✅ CRITICAL FIX: Use send time, not haptic generation time
#             bundled_msg.header.stamp = rospy.Time.now()  # ← CHANGED from msg.header
            
#             # Add pose data
#             bundled_msg.pose = msg.pose
            
#             # Add force timestamp echo
#             bundled_msg.force_timestamp = self.latest_force_timestamp
...
#             # Log force timestamp waiting time ONLY ONCE per unique timestamp
#             if self.latest_force_timestamp > 0 and self.latest_force_timestamp in self.force_timestamp_first_receive:
#                 receive_time = self.force_timestamp_first_receive[self.latest_force_timestamp]
#                 wait_time = sent_time - receive_time
...
#             rospy.loginfo_throttle(1.0, 
#                 f"Sent bundled @ {self.DESIRED_SAMPLING_RATE}Hz: "
#                 f"pose_send_time={sent_time:.6f}, force_ts_echo={self.latest_force_timestamp:.6f}")
...
#             with open(filepath, 'w', newline='') as csvfile:
#                 writer = csv.writer(csvfile)
#                 writer.writerow(["Timestamp from Slave", "Pose RTD (s)", "Pose RTD (ms)"])
#                 for timestamp, delay in self.pose_delay_entries:
#                     writer.writerow([f"{timestamp:.9f}", f"{delay:.9f}", f"{delay*1000:.3f}"])
            
#             # Print statistics
#             if self.pose_delays:
#                 rospy.loginfo("="*60)
#                 rospy.loginfo("Pose RTD Statistics:")
#                 rospy.loginfo(f"  Average: {sum(self.pose_delays)/len(self.pose_delays)*1000:.3f} ms")
#                 rospy.loginfo(f"  Min:     {min(self.pose_delays)*1000:.3f} ms")
#                 rospy.loginfo(f"  Max:     {max(self.pose_delays)*1000:.3f} ms")
#                 rospy.loginfo(f"  Total samples: {len(self.pose_delays)}")
#                 rospy.loginfo("="*60)
...
#             with open(filepath, 'w', newline='') as csvfile:
#                 writer = csv.writer(csvfile)
#                 writer.writerow(['Generation Time', 'Sent Time', 'Difference (s)', 'Difference (ms)'])
#                 for gen_time, sent_time, diff in self.pose_gen_send_log:
#                     writer.writerow([f"{gen_time:.9f}", f"{sent_time:.9f}", f"{diff:.9f}", f"{diff*1000:.3f}"])
#             rospy.loginfo(f"Pose gen→send log saved to: {filepath}")
#         except Exception as e:
#             rospy.logerr(f"Failed to write pose gen→send log to CSV: {e}")
...
#             with open(filepath, 'w', newline='') as csvfile:
#                 writer = csv.writer(csvfile)
#                 writer.writerow(['Receive Time', 'Applied Time', 'Difference (s)', 'Difference (ms)'])
#                 for recv, applied, diff in self.force_receive_log:
#                     writer.writerow([f"{recv:.9f}", f"{applied:.9f}", f"{diff:.9f}", f"{diff*1000:.3f}"])
#             rospy.loginfo(f"Force receive log saved to: {filepath}")
#         except Exception as e:
#             rospy.logerr(f"Failed to write force receive log to CSV: {e}")
...
#     def main_loop(self):
#         """
#         Main loop - spins to keep node alive.
...
#             #  CRITICAL FIX: Timestamp IMMEDIATELY before publish
#             send_timestamp = rospy.Time.now()
#             bundled_msg.header.stamp = send_timestamp
            
#             # Publish IMMEDIATELY after timestamping (minimize gap)
#             self.master_to_slave_pub.publish(bundled_msg)
            
#             # Now log (after publish to minimize delay)
#             sent_time = send_timestamp.to_sec()
#             gen_time = msg.header.stamp.to_sec()
...
#         # FIFO queue for force timestamps
#         self.force_timestamp_queue = deque()
...
#         rospy.loginfo(f"Master Controller initialized with {self.DESIRED_SAMPLING_RATE} Hz sampling rate (downsampling factor: {self.DOWNSAMPLE_FACTOR})")
#         rospy.loginfo("Using bundled messaging (MasterToSlaveData <-> SlaveToMasterData)")
#         rospy.loginfo("Force timestamp FIFO queue enabled")
...
#         # 3. Add force timestamp to FIFO queue (if valid and not duplicate)
#         force_timestamp = msg.force_timestamp
#         if force_timestamp > 0:  # Valid timestamp
#             # Only add if not already in queue (avoid duplicates)
#             if force_timestamp not in self.force_timestamp_queue:
#                 self.force_timestamp_queue.append(force_timestamp)
#                 rospy.loginfo_throttle(2.0, f"NEW force_timestamp queued: {force_timestamp:.6f} (queue size: {len(self.force_timestamp_queue)})")
            
#             # Track first receive time for logging
#             if force_timestamp not in self.force_timestamp_first_receive:
#                 self.force_timestamp_first_receive[force_timestamp] = rospy.get_time()
...
#             # Get force timestamp from FIFO queue (oldest first)
#             force_timestamp_to_send = None  # Default to None if queue is empty
#             if len(self.force_timestamp_queue) > 0:
#                 force_timestamp_to_send = self.force_timestamp_queue.popleft()
#                 rospy.loginfo_throttle(2.0, f"Sending force_ts from queue: {force_timestamp_to_send:.6f} (remaining: {len(self.force_timestamp_queue)})")
#                 # Only set the field if we have a valid timestamp
#                 bundled_msg.force_timestamp = force_timestamp_to_send
#             else:
#                 # Queue is empty - send default/uninitialized value (typically 0.0 in ROS)
#                 # Slave side should check if timestamp == 0.0 to know nothing was sent
#                 bundled_msg.force_timestamp = 0.0
#                 rospy.loginfo_throttle(5.0, "No force timestamp in queue - sending 0.0")
...
#             # Log force timestamp waiting time ONLY if we sent a valid timestamp
#             if force_timestamp_to_send is not None and force_timestamp_to_send > 0 and force_timestamp_to_send in self.force_timestamp_first_receive:
#                 receive_time = self.force_timestamp_first_receive[force_timestamp_to_send]
#                 wait_time = sent_time - receive_time
                
#                 self.force_timestamp_wait_log.append((
#                     force_timestamp_to_send,
#                     receive_time,
#                     sent_time,
#                     wait_time
#                 ))
                
#                 rospy.loginfo_throttle(2.0, f"Force_ts {force_timestamp_to_send:.6f} wait: {wait_time*1000:.3f}ms")
                
#                 # Remove from dictionary to avoid logging same timestamp again
#                 del self.force_timestamp_first_receive[force_timestamp_to_send]
...
#             rospy.loginfo_throttle(1.0, 
#                 f"Sent bundled @ {self.DESIRED_SAMPLING_RATE}Hz: "
#                 f"pose_send_time={sent_time:.6f}, force_ts_echo={force_timestamp_to_send if force_timestamp_to_send else 'None'}")
...
class MasterController:
    def __init__(self):
        ...
        self.force_ts_queue = deque()  # (force_timestamp, arrival_time)
        self.MAX_WAIT = 1.0 / self.DESIRED_SAMPLING_RATE
    ...
    def slave_to_master_callback(self, msg: SlaveToMasterData):
        ...
        if force_timestamp > 0:  # Valid timestamp
            ...
            if force_timestamp > 0:
                ...
                self.force_ts_queue.append((force_timestamp, arrival_time))
    ...
    def pose_callback(self, msg: PoseStamped):
        ...
        if self.sample_counter >= self.DOWNSAMPLE_FACTOR:
            ...
            # Add force timestamp echo
            now = rospy.get_time()
            force_ts_to_send = 0.0

            while self.force_ts_queue:
                ts, arrival = self.force_ts_queue[0]
                if now - arrival > self.MAX_WAIT:
                    self.force_ts_queue.popleft()  
                else:
                    break

# Pop oldest valid timestamp
            if self.force_ts_queue:
                ts, arrival = self.force_ts_queue.popleft()
                force_ts_to_send = ts
                self.latest_force_timestamp = ts
            bundled_msg.force_timestamp = force_ts_to_send
