...
class TDPAmaster:
    ...
    def TDPA_twoport(self, velocity, force, current_time, Es_in_delayed):
        if self.previous_time_of_msg is None:
            self.previous_time_of_msg = current_time
            return np.zeros((3,1)), np.zeros((3,1)), np.zeros((3,1))
        P = np.zeros((3,1))
        alpha = np.zeros((3,1))
        ...
        return smoothed_force, self.Em_in, velocity
        
    def publisher_callback(self, event):
        force, energy, velocity = self.TDPA_twoport(self.haptic_stylus_velocity, self.force_from_slave, rospy.get_time(), self.energy_from_slave)
        force_msg = OmniFeedback()
        force_msg.force.x, force_msg.force.y, force_msg.force.z = force[:3, 0]
        master_msg = Float64MultiArray()
        message = np.vstack((velocity[:3, 0], energy[:3, 0]))
