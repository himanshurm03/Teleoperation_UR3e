...
class MasterController:
    def __init__(self):
        ...
        self.pose_send_times = {}
        
        self.geomagic_pose_timestamps = []
    ...
    def pose_callback(self, msg: PoseStamped):
        ...
        # record every generated timestamp from Geomagic Touch
        gen_time = msg.header.stamp.to_sec()
        self.geomagic_pose_timestamps.append(gen_time)
    ...
    def time_from_slave_to_master_for_pose_callback(self, msg: Float64):
        ...
        sent_time = self.pose_send_times.pop(msg.data, None)

        if sent_time is not None:
            delay = recv_time - sent_time
            print(f"Pose RTD (round-trip delay): {delay:.6f} s")
            self.pose_delays.append(delay)
            self.pose_delay_entries.append((msg.data, delay))
        else:
            rospy.logwarn(f"Received unmatched pose timestamp: {msg.data}")
...
# new .csv file
    ...
    def save_geomagic_pose_timestamps_to_csv(self, filepath="/home/autonomous-lab/Desktop/delay/geomagic_pose_timestamps.csv"):
        try:
            os.makedirs(os.path.dirname(filepath), exist_ok=True)
            with open(filepath, 'w', newline='') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(["Geomagic Pose Generation Timestamp (s)"])
                for ts in self.geomagic_pose_timestamps:
                    writer.writerow([f"{ts:.9f}"])
            rospy.loginfo(f"Geomagic pose timestamps saved to: {filepath}")
        except Exception as e:
            rospy.logerr(f"Failed to write Geomagic pose timestamps to CSV: {e}")
    ...
    def shutdown_hook(self):
        ...
        self.save_geomagic_pose_timestamps_to_csv() 
    
    def main_loop(self):
        rate = rospy.Rate(500)  # can change to 1000, 500, 250 Hz
        while not rospy.is_shutdown():
            ...
            # Store sent timestamp for delay tracking
                self.pose_send_times[gen_time] = sent_time
