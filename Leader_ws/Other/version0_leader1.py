...
#ALL POSE TIMESTAMPS ARE SUBTRACTED FROM CURRENT TIMESTAMP.
...
#             current_time = rospy.get_time()   # SAME time for all
...
#                 self.pose_delay_entries.append(
#                     (pose_timestamp, pose_delay)
#                 )
...
#ONLY LATEST POSE TIMESTAMP WILL BE SUBTRACTED FROM THE CURRENT TIME
...
#         # 2. Calculate pose RTD (Round Trip Delay)
#         if msg.pose_timestamps:
#     # Take the LATEST pose timestamp only
#             latest_pose_timestamp = msg.pose_timestamps[-1]

#             current_time = rospy.get_time()
#             pose_delay = current_time - latest_pose_timestamp

#             print(f"Pose RTD (latest): {pose_delay*1000:.2f} ms")

#             self.pose_delays.append(pose_delay)
#             self.pose_delay_entries.append(
#                 (latest_pose_timestamp, pose_delay)
#             )
...
###################################################################################
#without fifo
#all timestamps subtracted from the current time
...
#without fifo
#latest pose timestamps subtracted from the current time
...
#         # 2. Calculate pose RTD (Round Trip Delay)
#         # 2. Calculate pose RTD (Round Trip Delay)
        # if msg.pose_timestamps:   # list is not empty
        #     latest_pose_timestamp = msg.pose_timestamps[-1]  # ✅ take ONLY latest
        #     current_time = rospy.get_time()
        #     pose_delay = current_time - latest_pose_timestamp

        #     print(f"Pose RTD (latest only): {pose_delay*1000:.2f} ms")

        #     self.pose_delays.append(pose_delay)
        #     self.pose_delay_entries.append((latest_pose_timestamp, pose_delay))
...
#ALL pose timestamps received from the slave is subtracted from the current time
...
#Only latest pose timestamp is subtracted from the current pose timestamp
