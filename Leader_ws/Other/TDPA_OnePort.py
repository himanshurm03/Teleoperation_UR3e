...
class TDPAmaster:
    ...
    def energy_function(self, force, velocity):
        if self.previous_time_of_msg is None:
            self.previous_time_of_msg = self.current_time_of_msg
            return
        self.energy_obs = self.energy_obs + np.multiply(force, -velocity)*(self.current_time_of_msg - self.previous_time_of_msg)
        self.previous_time_of_msg = self.current_time_of_msg
    ...
    def TDPA_oneport(self, pose, velocity):
        ...
        for i in range(3):
            if self.energy_obs[i,0]<0:
                alpha[i,0] = -self.energy_obs[i,0]/(delta_t*(velocity[i,0]**2))
                alpha[i, 0] = min(alpha[i, 0], 100) 
            else:
                alpha[i,0] = 0.0
        self.previous_time_of_msg = self.current_time_of_msg
        self.alpha_previous = alpha
        self.previous_haptic_stylus_velocity = velocity
        if alpha[0,0]!=0 or alpha[1,0]!=0 or alpha[2,0]!=0:
            print(f"alpha: {alpha}")
    ...
    def publisher_callback(self, event):
        if rospy.is_shutdown():
            return
        # force = self.force_function(self.haptic_stylus_position, self.haptic_stylus_velocity)
        # self.energy_function(force, self.haptic_stylus_velocity)
    ...
    def plot_data(self, data_type):
        times = np.array(self.time_stamps)
        if data_type == "pose":
            data_array = np.array(self.stylus_pose_plot)
            fig, axs = plt.subplots(3, 2, figsize=(16, 9))
            titles = ['Pose in x', 'Pose in y', 'Pose in z', 'Pose in roll', 'Pose in pitch', 'Pose in yaw']
            ylabel = 'Meters'  # For position
            ylabels = ['Meters', 'Meters', 'Meters', 'Radians', 'Radians', 'Radians']
            
            # Plot each pose component
            for i in range(6):
                axs[i // 2, i % 2].plot(times, data_array[:, i])
                axs[i // 2, i % 2].set_title(titles[i])
                axs[i // 2, i % 2].set_ylabel(ylabels[i])
                axs[i // 2, i % 2].set_xlabel('Seconds')
                axs[i // 2, i % 2].grid(True)

        elif data_type == "velocity":
            data_array = np.array(self.stylus_velocity_plot)
            fig, axs = plt.subplots(3, 1, figsize=(16, 9))
            titles = ['Velocity in x', 'Velocity in y', 'Velocity in z']
            
            # Plot each velocity component
            for i in range(3):
                axs[i].plot(times, data_array[:, i])
                axs[i].set_title(titles[i])
                axs[i].set_ylabel('Meters/second')
                axs[i].set_xlabel('Seconds')
                axs[i].grid(True)

        elif data_type == "force":
            data_array = np.array(self.stylus_force_plot)
            fig, axs = plt.subplots(3, 1, figsize=(16, 9))
            titles = ['Force in x', 'Force in y', 'Force in z']
            
            # Plot each force component
            for i in range(3):
                axs[i].plot(times, data_array[:, i])
                axs[i].set_title(titles[i])
                axs[i].set_ylabel('Newtons')  # Use appropriate unit for force
                axs[i].set_xlabel('Seconds')
                axs[i].grid(True)
